...
def bar_maker_ini(bar_height: int = 10, box: str = "█"):
    ...
    bar_height_and_color = {
        green_boxes: Fore.GREEN + box for green_boxes in first_items_that_are_green
    }
    bar_height_and_color.update(
        {
            yellow_boxes: Fore.YELLOW + box
            for yellow_boxes in first_items_that_are_yellow
        }
    )
    bar_height_and_color.update(
        {red_boxes: Fore.RED + box for red_boxes in first_items_that_are_red}
    )
...
def bar_maker(
    ...
    return [
        (
            bar_height_and_color[resource_value]
            if resource_value <= resource_usage
            else zero_fill * bar_width if resource_value == 1 else fill * bar_width
        )
...
def chart_parser(
    ...
    if chart_name != None:
        ...
        proper_chart.append(
            left_side + name_seperator * int(x / len(name_seperator)) + right_side
        )
    ...
    if resource_usage != None:
        proper_chart.append(
            left_side + value_seperator * int(x / len(value_seperator)) + right_side
        )
...
def av_cpu_ini():
    ...
    cpu_starting_chart: list = [
        [
            "." * cpu_bar_width if box == 0 else cpu_fill * cpu_bar_width
            for box in range(cpu_bar_height)
        ]
        for _ in range(cpu_chart_width)
    ]
    cpu_bar_height_and_color: dict = bar_maker_ini(
        bar_height=cpu_bar_height, box=cpu_box
    )
    return [
        {
...
def readwrite(section: str, key: str, value, data_type: type = str):
    ...
    while True:
        try:
            x = (
                config[section][key][1:-1]
                if re.search(r"^\".+\"$", config[section][key])
                else config[section][key]
            )
            return data_type(x)
        except KeyError:
            config[section] = {
...
def main():
    ...
    while True:
        ...
        av_cpu_last_chart = chart_maker(
            last_bar=bar_maker(
                resource_usage=int(
                    av_cpu_percent / 100 * av_cpu_values["cpu_bar_height"]
                ),
